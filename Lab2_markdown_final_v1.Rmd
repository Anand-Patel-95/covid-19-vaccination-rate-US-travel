---
title: "Lab2: Vaccination Rate and Tourism on the U.S. Continental West Coast"
author: "Anand Patel, Vijay Ranganatha, Viswanathan Thiagarajan"
date: "7/26/2021"
output:
  html_document:
    df_print: paged
...
All of the variables' distributions look much more normal now. The `County_Median_Income` still looks a bit right-skewed. We could go one step further down on the box-cox ladder of powers to transform this feature using a reciprocal root, but that comes at the cost of difficulty in interpreting the coefficient for `County_Median_Income`. We will choose to log base 10 transform all the variables.
...
Vijay - Add a table relating our final variable names to what they actually mean here
...
3 columns. 1st has name of variable in data as above. 2nd has names of variables used in latex below. 3rd is description.
...
Constants do not mean anything since we cannot have 0.

Have statistical significance.

### Practical Significance

Do not have practical significance b/c a 10% increase in vaccine rate (reasonable increase in the short term) means a 4.2% (model 3) or 5.6% (model 2) decrease in number of tourism trips taken. If Disney was a destination expecting 1000 visitors for in a week for example, then getting 944 visitors instead does not warrant significant action in terms of promotion or switching over to maintenance over operation.
...
Our analysis shows that tourism demand is slightly decreasing with rising vaccination rates, but not at a practically significant amount. 
- Since we have decrease in trips with vaccine rate, but the result is not practically significant:

- Do not promote tourism now because people are traveling less as vaccination rates increase slowly.
- Continue maintaining the tourism sites because demand is not growing and current tourism business operations since the decrease in demand is not large enough to stifle profits.
- Reevaluate promoting tourism in the future when the vaccination rates are much higher across counties. The Covid-19 climate in the West Coast will be different than now if all vaccination rates are above some threshold, like 70% for example.
